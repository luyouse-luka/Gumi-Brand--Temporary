#!/usr/bin/env python3
"""Three-way comparison before the r138 push, and the push list itself.

  baseline = baseline-20260910-r137   (live when this round started)
  remote   = prepush-r138             (live right now)
  local    = work-r138                (a copy of remote + this round's assets)
...
BASE, REMOTE, LOCAL = S/'baseline-20260910-r137', S/'prepush-r138', S/'work-r138'

def md5(p):
    return hashlib.md5(p.read_bytes()).hexdigest() if p.is_file() else None

if not (REMOTE/'sections').is_dir():
    sys.exit('ABORT: prepush-r138 is not a theme pull')
...
print(f'\ntheirs   ({len(theirs)}) since baseline-20260910-r137 was taken:')
...
print('shopify theme push --store je1ka9-er.myshopify.com --theme 180348977399 '
      '--nodelete --allow-live --path work-r138 \\\n    '
