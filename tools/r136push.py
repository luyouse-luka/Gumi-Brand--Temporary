#!/usr/bin/env python3
"""Three-way comparison before the r136 push, and the push list itself.

  baseline = baseline-20260909-r135   (live when this round started)
  remote   = prepush-r136             (live right now)
  local    = work-r136                (a copy of remote + this round's assets)
...
BASE, REMOTE, LOCAL = S/'baseline-20260909-r135', S/'prepush-r136', S/'work-r136'

def md5(p):
    return hashlib.md5(p.read_bytes()).hexdigest() if p.is_file() else None

if not (REMOTE/'sections').is_dir():
    sys.exit('ABORT: prepush-r136 is not a theme pull')
...
print(f'\ntheirs   ({len(theirs)}) since baseline-20260909-r135 was taken:')
...
print('shopify theme push --store je1ka9-er.myshopify.com --theme 180348977399 '
      '--nodelete --allow-live --path work-r136 \\\n    '
