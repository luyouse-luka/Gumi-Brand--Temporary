#!/usr/bin/env python3
"""Three-way comparison before the r135 push, and the push list itself.

  baseline = baseline-20260909-r134   (live when this round started)
  remote   = prepush-r135             (live right now)
  local    = work-r135                (a copy of remote + this round's assets)
...
BASE, REMOTE, LOCAL = S/'baseline-20260909-r134', S/'prepush-r135', S/'work-r135'

def md5(p):
    return hashlib.md5(p.read_bytes()).hexdigest() if p.is_file() else None

if not (REMOTE/'sections').is_dir():
    sys.exit('ABORT: prepush-r135 is not a theme pull')
...
print(f'\ntheirs   ({len(theirs)}) since baseline-20260909-r134 was taken:')
...
print('shopify theme push --store je1ka9-er.myshopify.com --theme 180348977399 '
      '--nodelete --allow-live --path work-r135 \\\n    '
