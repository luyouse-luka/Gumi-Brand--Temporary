...
| `sections/gb-header.liquid` | r90 | 菜单拆成两个 ul（桌面/手机各一），移除 `--mobile` 单项类 | ✅ **已推**（r94，2026-09-08，需求方明确要求现在就推）。推前与线上 diff 只有这一处菜单结构，对方期间没动过它。⚠ **后台 Mobile menu 仍是三项**，所以线上手机菜单当场从 6 项降到 Shop / Learn more / Get in Touch —— 需求方知情，补齐后台即恢复，见 `docs/LIVE-BACKLOG.md` 第〇节 |
