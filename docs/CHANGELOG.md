...
## 第一三八轮（2026-09-10）— placeholder 的号码长度按各国实际位数改（`$build` = `20260910-r138`）

需求方追问：「切换区号之后，后面的 placeholder 的号码长度对吗」。**不对。**

第一三七轮所有国家共用稿上澳洲那串 9 位数 `400 000 000`、只换区号前缀，
六个里错了四个：

| | r137 | 该国移动号实际位数 | 错在哪 |
|---|---|---|---|
| AU | `+61 400 000 000` | 9 | ✅ 稿值，正确 |
| NZ | `+64 400 000 000` | 8–9 | NZ **没有 400 号段**（实际 21/27 开头） |
| US | `+1 400 000 000` | **10** | 少 1 位 |
| CA | `+1 400 000 000` | **10** | 少 1 位 |
| GB | `+44 400 000 000` | **10** | 少 1 位 |
| SG | `+65 400 000 000` | **8** | 多 1 位 |

⚠ 这条 r137 就登记在「待确认」里了（各国号码样例需求方没给，**当时有意没编**）。
本轮需求方点名要改，才补上。

### 改法：每个 option 自带 `data-example`

```html
<option value="GB" data-dial="+44" data-example="7400 000000">GB</option>
```

`phoneCode` 的 `apply()` 改读 `data-example`，缺省时回退到原来那条共用尾数
（`data-example` 没写的 option 保持 markup 里的行为，不会静默变空）。

| | placeholder | 位数 |
|---|---|---|
| AU | `+61 400 000 000` | 9 ← 稿值未动 |
| NZ | `+64 21 000 0000` | 9 |
...
写法沿用稿上 AU 的路子：**真实号段前缀 + 全 0 尾数**。
⚠ **US / CA 用 `555`** —— 那是北美**保留给虚构用途**的交换码，
所以没有一个 placeholder 指向真实号码。`201`/`204` 是真实区号（新泽西 / 曼尼托巴），
只为让格式合法；**要绑到客服所在地就换这两个数**。

### 文件清单

```
改  get-in-touch.html / referral.html   六个 option 各加 data-example
改  liquid/sections/gb-form-section.liquid   仓库副本同改（线上那份单独改）
改  assets/main.js            apply() 改读 data-example，缺省回退旧行为
改  assets/customstyle.scss   仅 $build r137→r138（本轮无样式改动，
                              升它是给 main.js 的 ?v= 破缓存）
改  assets/customstyle.css    重新编译（0 条 Sass 警告）
改  *.html (12)               ?v= r137 → r138
改  tools/phonecode.py        期望表带位数 + 每国一条位数断言
新  tools/r138push.py         三方对比与推送清单
```
...
| 判据 | 结果 |
|---|---|
| `tools/phonecode.py` 本地 / 线上 | **34 / 0** 两端一致（比 r137 多 12 条：每国一条**位数断言**） |
| `--strip` 反向 | **20 红** |
| 390 档 placeholder 宽度 | 最长的 GB **160px / 可用 263px**，六个都放得下 |
| 回归 `cartsplit` / `inkringlive` | 5-0 / 9-0 |
| 三方对比 | ours 4 / theirs 0 / **CONFLICT 0** |
| 回读逐字节 | 4 个一致，清单外 **0**，624 → 624 |

新基线 **`baseline-20260910-r138`**（624 文件）。

### ⚠ 不要报成 bug

1. **`data-example` 不能合并回一条共用尾数** —— 位数按国家真的不同
   （SG 8 / AU、NZ 9 / US、CA、GB 10）。判据里那张表带位数就是钉这件事的，
   合并回去当场 12 条红。
2. **US 与 CA 的 `555` 不是占位符号而是有意选的号段** —— 北美保留给虚构用途，
   换成别的三位数就可能指向真实线路。
3. **`201` / `204` 是真实区号** —— 只为让格式合法，不代表业务所在地。
   要绑客服所在地，换这两个数即可（改完跑 `phonecode.py`，位数断言会守住长度）。
4. **`apply()` 里 `|| rest` 的回退不是死代码** —— 将来加国家若忘了写 `data-example`，
   回退到 markup 原有尾数，而不是把 placeholder 变成只剩一个区号。
5. **本轮 scss 只有 `$build` 一行变化，不是漏推样式**。
6. **没跑全站 `rwd`** —— 本轮只动 HTML 属性与 JS 一行，无 CSS 改动、不影响布局，
   而 placeholder 宽度已用 canvas 在 390 档单独实测（上表）。
