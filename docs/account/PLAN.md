...
- [x] **步骤 1：追加断言（先红）** —— 输入为空时 Apply 按钮的 `background-color`
      不等于主绿（值从 `31488` 取），填入后等于（值从 `31648` 取）；成功态 panel 高 292
- [x] **步骤 2：跑判据确认红**
- [x] **步骤 3：写三态，用 panel 上的 `data-acct-discount-state` 切**  ← 状态只有 `empty` / `applied` 两个：「已填」是输入框的值，
      不是第三份 DOM
- [x] **步骤 4：Apply 按钮的启用只看输入非空**（前端能判的就判；码是否真实存在要后端，不做）
- [x] **步骤 5：编译、跑判据确认绿、活性自检**
- [x] **步骤 6：提交** — `git commit -m "feat(account): 折扣码弹窗三态"`
...
另有 `2284:33129`「Container」390x750 —— ⚠ **不是**表单的滚动容器，是一份**旧草稿**：
它浮在板外、比弹窗里那份少一个字段（内容 686 而非 800）。真正的滚动容器是
`2284:33092`（390x528，`grow1` + `v:FILL`），`_Scroll bar` 是它的绝对定位兄弟。

⚠ **`32940` 与 `33161` 是同一个面板的两份数据**（逐节点相同，只差
`12 Charnwood Road / St Kilda / 3181` 与 `20 Park Avenue / Richmond / 3121`），
不是两个面板。只做一个 `shipping-form`，判据改这一个面板的值来验校验。

- [x] **步骤 1：追加断言（先红）** —— 51 条新断言，`check_shipping` 三段。
      ⚠ 三处问的答案与预期不同：**必填星号没有自己的颜色**（`33096` 的 Label
      没有 `characterStyleOverrides`，`*` 与标签同为 `#666666`，所以断言改成
      「标签里没有第二个元素」）；**`+61` 前缀是 29 宽的纯文字**，它旁边的
      chevron（`196:17798`）在板上 `visible=false`，不是国家选择器；滚动容器就是
      `.gb-acct-modal__body` 本身（`33092`），不需要新开一层
- [x] **步骤 2：跑判据确认红** —— 51 红
- [x] **步骤 3：写三态与表单校验**（必填、邮编格式；**不提交到任何后端**）。
      面板的 `__panel` 本身就是 `<form>`，校验交给浏览器自己的 `required` /
      `pattern="\d{4}"` —— 板上没有错误态，自画红框等于自拟视觉。链到成功页挂在
      `submit` 上而不是按钮的 `click`：只有校验通过才会有 `submit`
- [x] **步骤 4：滚动容器带 `data-lenis-prevent`** —— 不改 `main.js`（他方文件，
      且 PREVENT 只在 init 扫一次）。体与 **textarea** 两处都要带，textarea 最易漏
- [x] **步骤 5：编译、跑判据确认绿、活性自检** —— `1001 ok / 0 red`；两轮共 11 处突变，31 红
- [x] **步骤 6：提交** — `feat(account): 配送地址弹窗与表单校验`
